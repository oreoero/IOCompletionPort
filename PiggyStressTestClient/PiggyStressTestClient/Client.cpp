...
DWORD WINAPI CClient::_WorkerThread(LPVOID lpParam)
{
	...
	// 向服务器发送信息
	sprintf( szTemp,("First message %s"),pParams->szBuffer );
	...
	TRACE("向服务器发送信息成功: %s\n", szTemp);
	pClient->ShowMessage("Sending first message succeeded: %s", szTemp);


	// 再发送一条信息
	memset( szTemp,0,sizeof(szTemp) );
	sprintf( szTemp,("Second message:%s"),pParams->szBuffer );
	nBytesSent = send(pParams->sock, szTemp, strlen(szTemp), 0);
	if (SOCKET_ERROR == nBytesSent) 
	{
	...
	TRACE("向服务器发送信息成功: %s\n", szTemp);
	pClient->ShowMessage("Sending second message succeeded: %s", szTemp);

	
	// 发第3条信息
	memset( szTemp,0,sizeof(szTemp) );
	sprintf( szTemp,("Third message:%s"),pParams->szBuffer );
	nBytesSent = send(pParams->sock, szTemp, strlen(szTemp), 0);
	if (SOCKET_ERROR == nBytesSent) 
	{
	...
	TRACE("向服务器发送信息成功: %s\n", szTemp);
	pClient->ShowMessage("Sending third message succeeded: %s", szTemp);

	if( pParams->nThreadNo==pClient->m_nThreads )
	{
		pClient->ShowMessage(_T("Testing concurrent %d threads finished."),pClient->m_nThreads);
	}
...
bool  CClient::EstablishConnections()
{
	...
	for (int i=0; i<m_nThreads; i++)
	{
		...
		sprintf(m_pParamsWorker[i].szBuffer, "%d thread sending data %s", i+1, m_strMessage.GetString() );
